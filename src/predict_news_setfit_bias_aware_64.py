"""
Interactive fake-news prediction using the saved bias-aware 64-shot SetFit model.
...
MODEL_PATH = Path(
    "models/setfit_distilroberta_64_shot_bias_aware"
)
...
def find_temporal_warnings(text: str) -> list[str]:
    """Find obvious law-year and effective-year contradictions."""
    ...
    effective_years = [
        int(year)
        for year in re.findall(
            r"\b(?:come|comes|came|coming)\s+into\s+"
            r"(?:force|effect)\b.{0,80}?"
            r"\b((?:19|20)\d{2})\b",
            text,
            flags=re.IGNORECASE,
        )
    ...
    if act_years and effective_years:
        act_year = max(act_years)
        effective_year = min(effective_years)

        if effective_year < act_year:
            warnings.append(
                f"An Act dated {act_year} is described as coming "
                f"into force in {effective_year}."
            )
...
def load_model() -> tuple[SetFitModel, list[str]]:
    ...
    print("Loading saved bias-aware 64-shot SetFit model...")
...
def main() -> None:
    ...
    while True:
        ...
        temporal_warnings = find_temporal_warnings(
            cleaned_text
        )
        ...
        if temporal_warnings:
            ...
            for warning in temporal_warnings:
                print("Possible contradiction:", warning)

            print(
                "Verification status: REVIEW REQUIRED"
            )
            print(
                "This warning does not independently prove "
